...
dirs = [
    ...
    (r"1. 용병단(Mercenaries)\12. 자연의 방랑자 (Wanderers of Nature)", r"6-3. 대륙 용병단 (Mercenaries)\12. 자연 방랑자 (Wanderers of Nature)")
]

with open("copy_log_sync.txt", "w", encoding="utf-8") as f:
    f.write("STARTING...\n")
    for src, dst in dirs:
        ...
        if os.path.exists(sd):
            for filename in os.listdir(sd):
                if filename.endswith(".md"):
                    src_file = os.path.join(sd, filename)
                    dst_file = os.path.join(dd, filename)
                    os.makedirs(dd, exist_ok=True)
                    try:
                        shutil.copy2(src_file, dst_file)
                        f.write(f"COPIED: {filename} TO {dd}\n")
                    except Exception as e:
                        f.write(f"ERR FILE COPY: {str(e)}\n")
        else:
            f.write(f"ERR NOT FOUND: {sd}\n")
    f.write("DONE\n")
