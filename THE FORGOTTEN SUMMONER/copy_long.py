...
# Make absolute and prepend long path prefix
...
# We need a fallback without "\\?\" for the local log output just in case
log_path = os.path.join(r"C:\Users\Storm Credit\Documents\Obsidian Vault\THE FORGOTTEN SUMMONER", "python_copy_log.txt")

with open(log_path, "w", encoding="utf-8") as f:
    f.write("STARTING LONG PATH COPY\n")
    try:
        for src, dst in dirs:
            v1 = os.path.join(b, src, "9. 예하 부대 및 기사단 (Military Units)")
            v2 = os.path.join(b, src, "9. 예하 부대 (Military Units)")
            
            sd = v1 if os.path.exists(v1) else (v2 if os.path.exists(v2) else None)
