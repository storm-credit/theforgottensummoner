...
for src, dst in dirs:
    ...
    if sd:
        # Instead of shutil.rmtree, manually clean dummy files
        ...
        for item in os.listdir(sd):
            if item.endswith(".md"):
                sfile = os.path.join(sd, item)
                dfile = os.path.join(dd, item)
                try:
                    with open(sfile, "r", encoding="utf-8") as rf:
                        content = rf.read()
                    with open(dfile, "w", encoding="utf-8") as wf:
                        wf.write(content)
                except:
                    pass
