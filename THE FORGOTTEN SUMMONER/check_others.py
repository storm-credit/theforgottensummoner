import os, re

base_dir = r'C:\Users\Storm Credit\Documents\Obsidian Vault\THE FORGOTTEN SUMMONER\01. 아스트라리스 크로니클'
folders_to_check = ['01-31', '01-32', '01-33', '01-34', '01-39', '01-40', '01-41', '01-42']
pattern = re.compile(r'(패시브|버프|디버프|쿨타임|어그로|경험치|업그레이드|시스템|스탯|유저|에러|오류|에너지|출력|튜토리얼|해킹|서버|무적|기믹|데이터|모터|엔진|배터리|텔레포트|포탈|마나|초능력|포스|마도)')

with open('scan_others.txt', 'w', encoding='utf-8') as out:
    for folder_prefix in folders_to_check:
        target_folder = None
        for foldername in os.listdir(base_dir):
            if foldername.startswith(folder_prefix) and os.path.isdir(os.path.join(base_dir, foldername)):
                target_folder = os.path.join(base_dir, foldername)
                break
        
        if target_folder:
            found_words = set()
            for root, dirs, files in os.walk(target_folder):
                for filename in files:
                    if filename.endswith('.md'):
                        filepath = os.path.join(root, filename)
                        try:
                            with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
                                content = f.read()
                            found_words.update(pattern.findall(content))
                        except:
                            pass
            if found_words:
                out.write(f"{target_folder.split(os.sep)[-1]}: {len(found_words)}종 잔존 -> {list(found_words)}\n")
            else:
                out.write(f"{target_folder.split(os.sep)[-1]}: CLEAN\n")
