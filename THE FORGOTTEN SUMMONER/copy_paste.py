...
for b_sub, l_sub in dirs_to_copy:
    ...
    if os.path.exists(src_dir):
        ...
        try:
            ...
            success_count += 1
            print(f"COPIED: {dst_dir}")
        except Exception as e:
            print(f"ERR: {e}")
    else:
        # Fallback without 기사단
        src_dir_sh = os.path.join(backup_base, b_sub, "9. 예하 부대 (Military Units)")
        if os.path.exists(src_dir_sh):
            if os.path.exists(dst_dir):
                shutil.rmtree(dst_dir, ignore_errors=True)
            os.makedirs(os.path.dirname(dst_dir), exist_ok=True)
            try:
                shutil.copytree(src_dir_sh, dst_dir)
                success_count += 1
                print(f"COPIED: {dst_dir}")
            except Exception as e:
                print(f"ERR: {e}")
...
print(f"DONE {success_count} folders pasted.")
