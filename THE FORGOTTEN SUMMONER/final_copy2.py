...
with open(r"\\?\C:\Users\Storm Credit\Documents\Obsidian Vault\THE FORGOTTEN SUMMONER\python_final_log_utf8.txt", "w", encoding="utf-8") as f:
    for src, dst in dirs:
        ...
        if sd:
            ...
            for file in os.listdir(sd):
                if file.endswith(".md"):
                    try:
                        with open(os.path.join(sd, file), "r", encoding="utf-8") as rf:
                            c = rf.read()
                        with open(os.path.join(dd, file), "w", encoding="utf-8") as wf:
                            wf.write(c)
                    except Exception:
                        pass
            f.write(f"OK: {dst}\n")
