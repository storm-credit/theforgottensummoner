...
base_dir = r"C:\Users\Storm Credit\Documents\Obsidian Vault\THE FORGOTTEN SUMMONER\01. 아스트라리스 크로니클\01-8. 세력 아카이브 (국가·조직 정리)"
b = os.path.join(base_dir, "_Legacy_중립세력 (Backup)")
l = os.path.join(base_dir, "6. 범대륙 초국가 및 중립 세력 (Supranational & Neutral)")
...
with open("copy_log_absolute.txt", "w", encoding="utf-8") as log:
    for src, dst in dirs:
        sfull = os.path.join(b, src, "9. 예하 부대 및 기사단 (Military Units)")
        dfull = os.path.join(l, dst, "9. 예하 부대 및 기사단 (Military Units)")
        
        if os.path.exists(sfull):
            try:
                if os.path.exists(dfull):
                    shutil.rmtree(dfull, ignore_errors=True)
                shutil.copytree(sfull, dfull)
                log.write(f"COPIED: {sfull} -> {dfull}\n")
            except Exception as e:
                log.write(f"ERROR {sfull}: {str(e)}\n")
        else:
            sfull_alt = os.path.join(b, src, "9. 예하 부대 (Military Units)")
            if os.path.exists(sfull_alt):
                try:
                    if os.path.exists(dfull):
                        shutil.rmtree(dfull, ignore_errors=True)
                    shutil.copytree(sfull_alt, dfull)
                    log.write(f"COPIED (ALT): {sfull_alt} -> {dfull}\n")
                except Exception as e:
                    log.write(f"ERROR {sfull_alt}: {str(e)}\n")
            else:
                log.write(f"MISSING: {sfull} nor {sfull_alt}\n")
